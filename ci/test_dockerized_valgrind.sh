...
if [[ "$ARCHITECTURE" == i386 ]]; then
	...
	# immutable-cache tests don't even involve Fibers
	PHP_CONFIGURE_ARGS="$PHP_CONFIGURE_ARGS --disable-fiber-asm"
	CFLAGS="$CFLAGS -m32"
else
	PHP_IMAGE="php"
	DOCKER_IMAGE_VALGRIND="immutable-cache-valgrind-test-runner:$ARCHITECTURE-$PHP_VERSION"
fi

DOCKER_IMAGE_VALGRIND=immutable-cache-valgrind-test-runner:$PHP_VERSION_FULL
docker build \
